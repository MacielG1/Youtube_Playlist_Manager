        ...
        <SortableContext items={items}>
          <>
            ...
            {otherTypeVideos?.length > 0 && (
              <h2 className="mt-0 text-center font-semibold tracking-wide text-neutral-700 dark:text-zinc-300/90 xs:pl-16 xs:text-left sm:pl-24 ">{title}s</h2>
            )}
            <div className="mx-4 grid grid-cols-1 place-items-center gap-x-4 gap-y-0 pl-3 pt-1 xs:grid-cols-2 sm:pl-7 md:grid-cols-3 lg:mx-6 lg:grid-cols-4 2xl:mx-8 2xl:grid-cols-5 2xl:pt-2">
              {items?.map((playlist) => {
