...
export default function ImportExportTimers({ setModalOpen }: { setModalOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
  ...
  function importTimers(evt: React.ChangeEvent<HTMLInputElement>) {
    ...
    fileReader.onload = async (e: ProgressEvent<FileReader>) => {
      ...
      if (
        !Array.isArray(jsonData.savedPlaylists) ||
        !Array.isArray(jsonData.savedVideos) ||
        !Array.isArray(jsonData.allPlaylistData) ||
        !Array.isArray(jsonData.allVideoData)
      ) {
        toast.error("Invalid File", toastError);
        return;
      }
      ...
      setModalOpen(false);
      toast.success("Imported", toastSuccess);
    };
    ...
    <div className="flex w-full gap-2">
      ...
      {isExportable && (
        <button
          ...
          className="flex flex-1 cursor-pointer items-center justify-center gap-2 rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2 text-sm font-medium text-neutral-700 transition-all duration-150 hover:border-neutral-300 hover:bg-neutral-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 dark:border-neutral-600 dark:bg-neutral-800 dark:text-neutral-200 dark:hover:border-neutral-500 dark:hover:bg-neutral-700/50"
        >
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
